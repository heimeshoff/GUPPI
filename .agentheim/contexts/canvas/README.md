...
- **Missing tile** — the canvas visual for a registered-but-unwatched project (`ProjectSnapshot.missing: true`). Frame body at 50% opacity, border swapped from `frameBorder` to `statusMissing`, `✕` glyph at `spacing.lg` in the top-right corner of the header bar. `bcs: []` on the snapshot keeps the frame empty (the empty-frame placeholder is suppressed in this state); the frame is NOT filtered out of the per-project collection (the missing visual is the affordance). Right-click on the header bar still offers "Remove project". The project-title HTML overlay (below) reads the same `missing` flag and dims to 50% opacity in sync with the Pixi frame.
- **Project-title overlay** — the per-project frame title rendered as an **HTML overlay** (ADR-003 extension 2026-05-19) over the PixiJS canvas, NOT as a Pixi `Text`. Sized at a **constant** CSS pixel height (`var(--guppi-size-title)`) at every camera zoom — the Miro-style "frame title" affordance, where the overview is the use case that needs the title most. Subscribes to the same `camera` runes + `projects` `$state` + theme tokens the rest of the overlay layer uses, so pan/zoom/theme-flip/live-add reflows the title without a per-tick push from `renderScene`. End-truncates with CSS `text-overflow: ellipsis` bound to the current frame header width (`bcLayout.width * camera.zoom`). Lives in its own sub-layer (`z-index: 5`) inside the existing ADR-003 overlay container — ABOVE the canvas, BELOW context menus / toasts / modals — with `pointer-events: none` so it never swallows a pan / drag / right-click / wheel-zoom. BC titles, BC task counts, and the frame header's "N tasks" counts label are NOT overlay'd — they stay in Pixi `Text` and scale with the camera (smaller when zoomed out), gaining crispness from the DPR fix below.
- **Crispness invariant** (canvas-013) — the canvas-wide policy that every rendered element (frame borders, BC bubble borders, intra-project edges incl. arrowheads + ACL notches, project frame title text, BC bubble title text, BC task-count text, status badges, missing-tile glyph) renders at display resolution at every supported zoom. Three load-bearing pieces hold the invariant: (1) PixiJS booted with `app.init({ resolution: window.devicePixelRatio, autoDensity: true, … })` so Text and Graphics rasterise at the framebuffer's real resolution rather than the previous default 1× upscale; (2) stroke widths for the "borders" category (frame border, BC bubble border, header divider, focus ring, and all four intra-project edge variants incl. arrowheads + ACL notch geometry) are **constant screen-space CSS pixels** — the previous `Math.max(1, shape.borderWidth* * z)` pattern that produced sub-pixel hairlines at zoom-out and chunky strokes at zoom-in is retired; (3) project frame titles ride the HTML overlay above. Focus-ring **inset** (positional offset) stays world-space; only the stroke width is constant-screen-space. Arrowhead `pullBack` (the tip-from-bubble distance) stays world-space because the bubble it pulls back from is `bcInsideWidth * z` in screen space.
