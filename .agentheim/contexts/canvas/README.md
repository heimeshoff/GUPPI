...
- **Bubble** — visual representation of a bounded context, drawn INSIDE its project frame (§3.7 inside-frame variant of the BC node). Denser than the orbit-baseline BC node: title + counts pill in one row at default zoom, status badge slot retained for `agent-awareness`. *Superseded by the kanban-accordion pivot (ADR-017):* the frame interior is no longer a cloud of Pixi BC bubbles but a vertical accordion of collapsible BCs rendered as DOM overlays; a BC is now an accordion section holding a kanban, not a draggable bubble. This vocabulary entry is historical pending the interior rebuild (canvas-020/021/022).
