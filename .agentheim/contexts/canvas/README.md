...
- **Frame** — the surrounding visual region drawn around a project (`canvas-007`). Replaces the orbit-baseline single-bubble tile: project name, status badges, and task counts ride a header bar across the top edge. The header bar is the project's drag handle and right-click target. *Pivot (ADR-017 / ADR-019, canvas-020):* the frame is now **hybrid** — the **shell** (fill + header + drag hit-area + missing glyph) is a PixiJS world-space object; the **interior** (the BC accordion + kanban) is a DOM overlay positioned at `worldToScreen(frame.pos)` + `transform: scale(z)`. The shell is a **fixed DIN-A4-ratio sheet** (ADR-019 amendment 2026-05-24, superseding the original fixed-shell stance): `frame-interior.frameSize` width fits all four kanban columns at once and height = `width × √2` (`FRAME_ASPECT_RATIO`); the interior **scrolls within** the sheet when taller. The four columns are `flex: 1 1 0; min-width: 0` so they share the board width equally and shrink to fit — `.kanban-board` is `overflow: hidden`, so there is never a horizontal scrollbar (the DONE column is never clipped). **No orange boundary:** a normal frame has no border (a MISSING project keeps its signal border) and no orange hover ring — the edge is a slight drop shadow (`--guppi-frame-shadow`, lifting on hover via `hoveredFrameId`); each BC accordion row gets the same `--guppi-accordion-shadow` instead of an orange hover outline. **Dragging:** a transparent DOM `.frame-header-handle` over the header band drives the frame drag (deterministic; the Pixi-drawn title shows through), mounted for **every on-screen frame** (`framesOnScreen`); when zoomed out past the LOD floor the handle takes its `.full` variant and covers the whole sheet, so the frame stays draggable with the board hidden. **Middle mouse button** always pans the canvas (even over a frame); **wheel** zooms no matter where the cursor is, except **ctrl + wheel over a BC** scrolls that BC's content vertically (up/down) instead of zooming.
- **Frame interior** — the DOM-overlay half of the hybrid frame (ADR-017 / ADR-019, canvas-020). A vertical **BC accordion**; each row expands to a four-column **kanban board**. The overlay (and its drag handle) mounts for **every on-screen frame** (`framesOnScreen` `$derived`, viewport culling `CULL_MARGIN_PX = 120` only); the heavy kanban **body** mounts only above the zoom-floor LOD (`LOD_ZOOM_FLOOR = 0.45`, `view.mountInterior`), so a zoomed-out frame is still draggable but doesn't pay for its card DOM. Recomputed on every camera change via the `cameraVersion` reactive-bridge counter (ADR-019). Pure cull/LOD/bucketing/`frameSize` helpers live in `src/lib/frame-interior.ts` (the tested verification surface).
