...
<!-- done-list:start -->
- [design-system-006-kanban-accordion-card-and-panel-vocabulary](done/design-system-006-kanban-accordion-card-and-panel-vocabulary.md) — `feature` — Styleguide GATE for the canvas pivot. New `STYLEGUIDE.md` §3.9 (BC accordion row), §3.10 (kanban board/column), §3.11 (task card — default/hover/selected/blocked + live-agent indicator line), §3.12 (docked detail panel + "AGENT NEEDS AN ANSWER" callout + slide-in/out motion), each citing tokens by name. New tokens mirrored across `tokens.ts` + `tokens.css` with `colorDark`/`colorLight` parity + a `[data-theme="light"]` block (design-system-004 dual-palette pattern); reuses the §2.2 status palette + §2.6 motion. §3.7 (BC bubble) + §3.8 (intra-project edges) annotated as superseded for the canvas interior (pointers to §3.9–3.12 + canvas-019/ADR-017), vocabulary preserved not deleted. New `references/kanban-accordion-sketch.md` ASCII layout reference. `pnpm check` 0/0/0; no Rust touched. Verifier: PASS iter 1. (2026-05-24)
