...
<!-- done-list:start -->
- [project-registry-005-individual-task-records-in-snapshot](done/project-registry-005-individual-task-records-in-snapshot.md) — `feature` — Grew the read-model from per-BC `task_counts` to individual task records: new `Task { id, title, column, type_, tags, blocked_question }` (mirrored in `types.ts`), `BoundedContext.tasks: Vec<Task>` with counts now derived; `get_project` reads each task file's frontmatter (malformed degrades to filename-stem id + empty metadata, never aborts). `TaskAdded` payload extended with title/type_/tags; new `TaskChanged` variant fires on in-place frontmatter edits without a column move (real-FS unit test); `TaskMoved` regression held. ADR-009 gains the variant + reconciliation note; `snapshot-patch.ts` patches cards in place under `guppi://event`. `cargo test --lib` 132 green; `pnpm check` 0/0/0; 33 frontend tests pass. Verifier: PASS iter 1. (2026-05-24)
