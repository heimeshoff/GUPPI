...
## Purpose
guppi is a personal mission-control for managing development projects that use the AgentHeim plugin and Claude Code. It gives me a single Miro-like canvas where every AgentHeim project is visible at once — projects as tiles, bounded contexts as connected child nodes, with live indicators of what's running and what needs attention. I drive it primarily by voice ("Guppy, refine task 104 in image-gallery") with mouse and keyboard as fallbacks. It launches `claude` *inside each target project's folder* so the project's own skills, hooks, and rules apply — guppi is a controller, not a host.
...
- Voice input: "Guppy, …" wake word via Whisperheim, fuzzy command parsing, all AgentHeim actions accessible by voice; voice-driven focus ("zoom to image-gallery")
...
- **Wake word** — "Guppy", spoken to begin a voice command.
